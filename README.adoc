...
JJWT is a pure Java implementation based exclusively on the https://tools.ietf.org/html/rfc7519[JWT],
https://tools.ietf.org/html/rfc7515[JWS], https://tools.ietf.org/html/rfc7516[JWE],
https://tools.ietf.org/html/rfc7518[JWA], https://tools.ietf.org/html/rfc7517[JWK],
https://www.rfc-editor.org/rfc/rfc7638.html[JWK Thumbprint], and
https://www.rfc-editor.org/rfc/rfc9278.html[JWK Thumbprint URI] RFC specifications and
open source under the terms of the http://www.apache.org/licenses/LICENSE-2.0[Apache 2.0 License].

The library was created by https://github.com/lhazlewood[Les Hazlewood]
...
IMPORTANT: this one is important.

Block form admonition (from adoc ref)
...
[source, java]
----
System.out.println("Hello JJWT!");
System.out.println("call out test"); // <.>
----
<.> This is a simple println to test how callouts look when rendered in GH.
====
...
Manually formatted table of contents
 ...
 ** <<compression-custom,Custom Compression Codec>>
 ** <<compression-custom-locator,Custom Compression Codec Locator>>
* <<json,JSON Processor>>
...
about something, please https://stackoverflow.com/questions/ask?tags=jjwt&guided=false[ask your question here].

After asking your question, you may wish to join our https://jwtk.slack.com/messages/CBNACTN3A[Slack] or
https://gitter.im/jwtk/jjwt[Gittr] chat rooms, but note that they may not always be attended. You will usually
have a better chance of having your question answered by
https://stackoverflow.com/questions/ask?tags=jjwt&guided=false[asking your question here].
...
https://stackoverflow.com/questions/ask?tags=jjwt&guided=false[ask your question here], or try Slack or Gittr as
described above.
...
please do https://github.com/jwtk/jjwt/issues/new[create a new JJWT issue].
...
without https://github.com/jwtk/jjwt/issues/new[creating a new JJWT issue] and discussing your desired
...
So, please https://github.com/jwtk/jjwt/issues/new[create a new JJWT issue] first to discuss, and then we can see if
...
. Call builder methods to set the `payload` <<jwt-content,content>> or <<jwt-claims,claims>> and any
<<jwt-header-builder,`header` parameters>> as desired.
. Optionally call `signWith` or `encryptWith` methods if you want to digitally sign or encrypt the JWT, respectively.
...
* `setIssuer`: sets the https://tools.ietf.org/html/rfc7519#section-4.1.1[`iss` (Issuer) Claim]
* `setSubject`: sets the https://tools.ietf.org/html/rfc7519#section-4.1.2[`sub` (Subject) Claim]
* `setAudience`: sets the https://tools.ietf.org/html/rfc7519#section-4.1.3[`aud` (Audience) Claim]
* `setExpiration`: sets the https://tools.ietf.org/html/rfc7519#section-4.1.4[`exp` (Expiration Time) Claim]
* `setNotBefore`: sets the https://tools.ietf.org/html/rfc7519#section-4.1.5[`nbf` (Not Before) Claim]
* `setIssuedAt`: sets the https://tools.ietf.org/html/rfc7519#section-4.1.6[`iat` (Issued At) Claim]
* `setId`: sets the https://tools.ietf.org/html/rfc7519#section-4.1.7[`jti` (JWT ID) Claim]
...
. Use the `Jwts.parserBuilder()` method to create a `JwtParserBuilder` instance.
. Optionally call `setKeyLocator`, `verifyWith` or `decryptWith` methods if you expect to parse <<jws,signed>> or <<jwe,encrypted>> JWTs.
...
If you are able, prefer creating a <<jws-key-create-secret,new secure-random secret key>> instead.
...
. Use the `Jwts.parserBuilder()` method to create a `JwtParserBuilder` instance.
. Call either <<key-locator,setKeyLocator>> or `verifyWith` methods to determine the key used to verify the JWS signature.
...
. Use the `Jwts.parserBuilder()` method to create a `JwtParserBuilder` instance.
. Call either <<key-locator,setKeyLocator>> or `decryptWith` methods to determine the key used to decrypt the JWE.
...
. Call the `forKey` method with the Java key you wish to represent as a JWK.
. Call builder methods to set any additional key fields or metadata, such as a `kid` (Key ID), X509 Certificates,
etc as desired.
